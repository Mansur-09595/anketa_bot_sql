...
bot = telebot.TeleBot("1046571094:AAHEYTQwblsSftwrvmaQ-vE6Ta9oJmIGiOw")
...
try:
  ...
  if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
    print("Что-то то не так с вашим именем пользоватея или паролем")
    sys.exit()
...
#     first_name VARCHAR(255), last_name VARCHAR(255), description \
#         VARCHAR(255), user_id INT(11))")
...
def process_description_step(message):
    try:
        ...
        bot.send_message(-1001275055621, user.first_name + ' ' + user.last_name + ' ' + user.description)
    except Exception as e:
        bot.reply_to(message, 'oooops')
...
#     group_id = -1001275055621




# # # Очистка БД # # #
# ("DELETE FROM categories") or 
# cursor.execute("DROP TABLE IF EXISTS categories")
# db.commit()

# # # Узнать ID # # #
# print(bot.get_chat('@reqistertest').id)